...
namespace Microsoft.DotNet.ToolPackage.Tests
{
    ...
    /// </summary>
    public class HttpRetryHandlerWithProgress : IHttpRetryHandler
    {
        public HttpRetryHandlerWithProgress(DownloadProcessMonitor downloadProcessMonitor) =>
            _downloadProcessMonitor =
                downloadProcessMonitor ?? throw new ArgumentNullException(nameof(downloadProcessMonitor));
            ...
            CancellationToken cancellationToken)
        {
            ...
            while (tries < request.MaxTries && !success)
            {
                ...
                using (var requestMessage = request.RequestFactory())
                {
                    ...
                    requestMessage.Options.Set(new HttpRequestOptionsKey<List<Stopwatch>>(StopwatchPropertyName),
                        stopwatches);
                    ...
                    try
                    {
                        ...
                        response = await TimeoutUtility.StartWithTimeout(
                            async timeoutToken =>
                            {
                                ...
                                var responseMessage = await request.HttpClient.SendAsync(requestMessage,
                                    request.CompletionOption, timeoutToken);
                        ...
                        if (response.Content != null)
                        {
                            ...
                            var timeoutStream = new DownloadTimeoutStreamWithLog(
                                totalDownloadSize,
                                _downloadProcessMonitor,
                                log,
                                requestUri.ToString(),
                            ...
                            var newContent = new StreamContent(timeoutStream);
